...
const products = [
  // Fruits (6 products)
  {
    ...
    unit: '1 lb',
    image:
      'https://res.cloudinary.com/abdulhammed/image/upload/v1748517345/big-cart/apple.jpg',
    status: null,
...
async function updateProductImages() {
  ...
  try {
    ...
    const existingProducts = response.documents;

    // Loop through each product in the array
    for (const product of products) {
      // Find the matching document by name and category
      const existingProduct = existingProducts.find(
        (doc) => doc.name === product.name && doc.category === product.category,
      );

      if (existingProduct) {
        // Update only the image field for the matching document
        await databases.updateDocument(
          databaseId,
          collectionId,
          existingProduct.$id, // Document ID from Appwrite
          {
            image: product.image,
          },
        );
        console.log(
          `Updated image for product: ${product.name} in category ${product.category}`,
        );
      } else {
        console.log(
          `Product not found: ${product.name} in category ${product.category}, skipping...`,
        );
      }
    }
